...
HOST=mongodb
PROJECT_DIRECTORY=$2
FILES=$(ls ${PROJECT_DIRECTORY}/*.json | sort -n -t _ -k 2)
...
do
    ...
    mongoimport --host ${HOST} --username productListUser --password productListPassword --authenticationDatabase admin --db promotions --collection ${COLLECTION} --drop --file ${AFILE}
